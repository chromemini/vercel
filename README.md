...
1. Accepts a `POST` from the browser with the user's Cloudflare credentials in custom headers.
...
**BYOK is preserved end-to-end.** Credentials are read per-request from headers, never stored, logged, or persisted server-side.
...
| `X-CF-Account-Id` | User's 32-char Cloudflare Account ID |
| `X-CF-Api-Token` | User's Cloudflare API Token (Workers AI permission) |

### Request body

Standard OpenAI-compatible chat completion payload — passed through verbatim to Cloudflare:
...
- **No secrets on the server.** The bridge reads the user's Account ID and API Token per request and never persists them. There is nothing to leak from Vercel's side.
- **Open CORS (`*`) is intentional.** The bridge is a public utility; credentials travel in headers supplied by the caller. Any origin can call it, but without valid Cloudflare credentials in the headers, the request fails at Cloudflare with 401/403.
- **Account ID format is validated** (`^[a-f0-9]{32}$`) to prevent header injection or malformed upstream URLs.
